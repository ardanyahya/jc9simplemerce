...
class ManageProduct extends Component {
    ...
    //Delete product dari product list
    deleteProduct = (id) => {
        axios.delete(
            `http://localhost:2019/products/${id}`
        ).then(res =>{
            ...
            if(item.id !== this.state.selectedId){
                return (
                    <tr>
                        ...
                        <td>
                            <button className = 'btn btn-primary' onClick={() => {this.setState({selectedId: item.id})}} >Edit</button>
                            <button className = 'btn btn-warning'  onClick={() => this.deleteProduct(item.id)} >Delete</button>
                        </td>
                    </tr>
                )
            } else {
