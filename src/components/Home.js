...
class Home extends Component {
    ...
    //Membuat tombol search memfilter harga max atau min
    onBtnSearch = () => {
        const name = this.name.value
        const min = parseInt(this.min.value) //NaN
        const max = parseInt(this.max.value) //NaN
        //memfilter nama product
        //tolowercase agar kata yang dinput user menjadi kecil semua
        ...
        var arrSearch = this.state.searchProducts.filter(item => {
            ...
            } else if (isNaN(min)) {//Name dan Max
                return(
                    item.name.toLowerCase().includes(name.toLowerCase())
                    &&
                    item.price <= max
